...
with seeds 110 and 2026. This tuner evaluates those qualifying starts plus a
stress-test spawn and rejects parameter sets that brake, touch a wall, take
damage, fail to complete a lap, or fail to survive.
"""
...
SCENARIOS = ((110, 1), (2026, 1), (110, 10), (2026, 7))
...
class SoloEvaluator:
   ...
   def run(self, *, seed: int, race_index: int = 1, duration_seconds: float = 30.0) -> TrialMetrics:
...
def sampled_parameters(trial: optuna.Trial) -> dict[str, float]:
   return {
       "CENTER_WEIGHT": trial.suggest_float("CENTER_WEIGHT", -0.22, 0.0),
       "NEAR_LOOKAHEAD_WEIGHT": trial.suggest_float("NEAR_LOOKAHEAD_WEIGHT", -0.35, -0.08),
       "MID_LOOKAHEAD_WEIGHT": trial.suggest_float("MID_LOOKAHEAD_WEIGHT", 0.38, 0.80),
       "FAR_LOOKAHEAD_WEIGHT": trial.suggest_float("FAR_LOOKAHEAD_WEIGHT", -0.05, 0.15),
       "HEADING_DIVISOR": trial.suggest_float("HEADING_DIVISOR", 180.0, 600.0),
       "STEERING_GAIN": trial.suggest_float("STEERING_GAIN", 0.70, 1.30),
       "BASE_SPEED": trial.suggest_float("BASE_SPEED", 22.8, 27.0),
       "TURN_SLOWDOWN": trial.suggest_float("TURN_SLOWDOWN", 1.0, 4.0),
       ...
       "FRONT_SLOW_DISTANCE": trial.suggest_float("FRONT_SLOW_DISTANCE", 1.0, 5.0),
       "FRONT_SPEED_SCALE": trial.suggest_float("FRONT_SPEED_SCALE", 0.8, 5.0),
...
def main() -> None:
   ...
   def objective(trial: optuna.Trial) -> float:
       ...
       metrics = tuple(evaluator.run(seed=seed, race_index=race_index) for seed, race_index in SCENARIOS)
